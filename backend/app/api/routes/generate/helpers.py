...
def _format_response(
    ...
    # dynamic role archetypes generated by ArchetypeGenerator when
    # they're present in benchmark_data. When the upstream flag is
    # off the key is simply absent (preserves all existing FE
    # consumers — see frontend/src/lib/firestore/models.ts
    # Benchmark type).
    _archetypes = benchmark_data.get("archetypes")
    if isinstance(_archetypes, list) and _archetypes:
        benchmark["archetypes"] = _archetypes
