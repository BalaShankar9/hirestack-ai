...
def test_metrics_endpoint_exposes_all_six_families():
    ...
    Post m11-pr41 the exposition is generated by ``prometheus_client``
    via ``app.core.prometheus_collectors.render_metrics()``. Source
    inspection moves to that module, and we additionally verify the
    rendered bytes contain each family name.
    """
    import inspect
    from app.core import prometheus_collectors as pc

    src = inspect.getsource(pc)
    body, _ct = pc.render_metrics()
    ...
    for marker in (
        ...
        assert marker in body_str, f"/metrics output missing m11-pr38 family: {marker}"
