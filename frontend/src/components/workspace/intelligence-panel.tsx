...
function buildInsights(
  ...
  // ATLAS v2 — dynamic archetypes (Slice 4.2)
  const archetypes = Array.isArray(app.benchmark?.archetypes)
    ? app.benchmark!.archetypes!
    : [];
  if (archetypes.length > 0) {
    items.push({
      icon: <Sparkles className="h-3.5 w-3.5" />,
      label: `${archetypes.length} target archetype${archetypes.length !== 1 ? "s" : ""} generated by ATLAS`,
      color: "text-violet-600 bg-violet-500/10",
    });
  }

  // ATLAS v2 — candidate claim validation (Slice 4.2)
  const validation = app.meta?.atlas_candidate_validation;
  if (validation && Array.isArray(validation.claims) && validation.claims.length > 0) {
    const v = validation.verified_count ?? 0;
    const c = validation.conflicted_count ?? 0;
    const total = validation.claims.length;
    items.push({
      icon: <Shield className="h-3.5 w-3.5" />,
      label:
        c > 0
          ? `Claim validation: ${v}/${total} verified — ${c} conflict${c !== 1 ? "s" : ""}`
          : `Claim validation: ${v}/${total} verified`,
      color:
        c > 0
...
export function IntelligencePanel({
  ...
  const archetypes = app.benchmark?.archetypes;
  const validation = app.meta?.atlas_candidate_validation;
  const hasAtlasExtras =
    (Array.isArray(archetypes) && archetypes.length > 0) ||
    (validation && Array.isArray(validation.claims) && validation.claims.length > 0);

  if (insights.length === 0 && !hasAtlasExtras) {
    return (
    ...
    <div className="surface-premium rounded-2xl p-4 card-spotlight">
      ...
      {hasAtlasExtras && (
        <div className="mt-4 space-y-4 border-t pt-4">
          ...
          <CandidateValidationPanel report={validation} />
        </div>
      )}
    </div>
  );
