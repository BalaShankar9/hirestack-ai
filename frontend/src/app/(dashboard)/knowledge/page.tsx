    ...
    >
      ...
      <div className="flex items-center gap-3 text-2xs text-muted-foreground mb-3">
        ...
        {resource.rating_count > 0 && (
          <span className="flex items-center gap-1">
            <Star className="h-3 w-3 text-amber-500" /> {resource.rating_avg.toFixed(1)}
          </span>
        )}
...
export default function KnowledgeLibraryPage() {
  ...
  useEffect(() => { loadProgress(); loadRecommendations(); }, [loadProgress, loadRecommendations]);
  ...
  return (
    <div className="container max-w-6xl py-8 space-y-6">
      ...
      <div className="flex items-center gap-1 border-b pb-0">
        {(["browse", "saved", "recommended"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={cn(
          ...
          >
            {t === "browse" ? "Browse" : t === "saved" ? "My Library" : "Recommended"}
