...
const STATUS_COLORS: Record<string, string> = {
  ...
  saved: "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300",
};
    ...
    <div className="space-y-8 p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold">Job Board Sync</h1>
            <p className="text-muted-foreground">AI-scored job matches based on your profile</p>
          </div>
        </div>
      ...
      {alerts.length > 0 && (
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-3">
            {alerts.map((a) => (
              <div key={a.id} className="flex items-center gap-2 rounded-full border px-4 py-2 text-sm">
      ...
      <div className="space-y-4">
        ...
        {loading && matches.length === 0 ? (
          ...
          <div className="space-y-4">
            {matches.map((m) => (
              <div key={m.id} className="rounded-xl border p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center gap-3">
                      <h3 className="font-semibold text-lg">{m.title || "Untitled Position"}</h3>
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[m.status || "new"] || STATUS_COLORS.new}`}>
                        {m.status || "new"}
                      </span>
                    </div>
                  ...
                  <div className="text-center shrink-0">
                    <div className={`text-3xl font-bold ${
                      (m.match_score || 0) >= 80 ? "text-green-600" : (m.match_score || 0) >= 60 ? "text-yellow-600" : "text-red-600"
                    }`}>
                      {m.match_score || 0}
                    </div>
                ...
                <div className="flex items-center gap-2 mt-4 pt-3 border-t">
                  ...
                  <Button size="sm" variant="outline" onClick={() => updateStatus(m.id, "rejected")} className="text-red-500">
