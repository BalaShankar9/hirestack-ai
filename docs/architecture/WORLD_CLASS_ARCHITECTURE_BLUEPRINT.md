...
| P1-9 | Centralized feature-flag service with audit | — | platform | SHIPPED (m12-pr09) — registry + sunset-CI from earlier; `public.feature_flag_audit` table + `FeatureFlagAuditService` (`record_change`, `record_snapshot_from_registry`, `get_latest`, `get_history`) provide append-only history; idempotent (skips when value unchanged); admin/runtime flips and per-deploy snapshots both write to the same audit trail |
...
| P1-12 | Adversarial prompt-injection defense (pre-classifier + structural separation) | W15 | ai-team + security | SHIPPED (m12-pr10) — `wrap_user_input` provided structural separation; new `prompt_injection_classifier` adds weighted multi-signal pre-classifier (override / role-impersonation / ChatML / tag-breakout / exfil-probe / tool-abuse / encoded-blob) returning allow/redact/block verdicts; `classify_and_wrap` is the one-call helper for prompt builders; observe-mode by default, env-tunable thresholds, telemetry hook for non-allow verdicts |
